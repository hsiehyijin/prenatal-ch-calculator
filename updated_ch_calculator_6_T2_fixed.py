...
with col1:
    ...
    age_1st = st.number_input("Maternal Age", min_value=15.0, max_value=55.0, value=32.0, key="age1")
    ...
    st.markdown("<div style='background-color: #e0f7fa; padding: 10px; border-radius: 5px;'><b style='color: red;'>Select 1st trimester outcome:</b></div>", unsafe_allow_html=True)
...
with col2:
    st.subheader("Second Trimester Calculator")
    st.caption("Trained on 66 non-aneuploidy cases with CH in second trimester")

    nt_2nd = st.number_input("NT (mm, T2)", min_value=0.1, max_value=10.0, value=2.5, key="nt2")
    crl_2nd = st.number_input("CRL (mm, T2)", min_value=5.0, max_value=100.0, value=50.0, key="crl2")
    resolved_ch = st.selectbox("Resolved CH?", ["Yes", "No"], key="resolvedCH")

    st.markdown("<div style='background-color: #fff1e2; padding: 10px; border-radius: 5px;'><b style='color: red;'>Select 2nd trimester outcome:</b></div>", unsafe_allow_html=True)
